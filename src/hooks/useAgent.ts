...
export const useAgent = (options: UseAgentOptions = {}) => {
  ...
  const callAgent = useCallback(async (callOptions: AgentCallOptions): Promise<AgentResponse> => {
    ...
    try {
      ...
      const data = res.ok ? {
        success: true,
        response: rawData.response || rawData,
        agent_id: rawData.agent_id || finalAgentId,
        ...
        success: false,
        error: rawData.error || rawData.message || `API returned status ${res.status}`,
...
export const callAgentAPI = async (
  ...
  try {
    ...
    const data = res.ok ? {
      success: true,
      response: rawData.response || rawData,
      agent_id: rawData.agent_id || agentId,
      user_id: rawData.user_id || options?.userId,
      session_id: rawData.session_id || options?.sessionId,
      timestamp: rawData.timestamp || new Date().toISOString(),
      raw_response: JSON.stringify(rawData),
    } : {
      success: false,
